...
@pytest.fixture
def make_endpoint_require_admin(app):
...
def test_app_require_admin_unauthorized(make_endpoint_require_admin, db, client):
    assert client.get(url_for('endpoint_require_admin')).status_code == 401


def test_app_require_admin_not_an_admin(make_endpoint_require_admin, valid_auth_header, client):
    assert client.get(url_for('endpoint_require_admin'), headers=[valid_auth_header]).status_code == 401


def test_app_require_admin_authorized(make_endpoint_require_admin, valid_admin_auth_header, client):
    assert client.get(url_for('endpoint_require_admin'), headers=[valid_admin_auth_header]).status_code == 200
...
def test_new_group_require_admin(valid_auth_header, client):
    res = client.post(url_for('api.new_group'), headers=[valid_auth_header],
                      data=json.dumps({'name': 'test_group'}),
                      content_type='application/json')
    assert res.status_code == 401


def test_new_group_create(valid_admin_auth_header, db, client):
    group_name = 'test_group'

    assert Group.query.count() == 0

    res = client.post(url_for('api.new_group'), headers=[valid_admin_auth_header],
